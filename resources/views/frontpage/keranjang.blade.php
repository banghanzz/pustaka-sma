            ...
            <table class="table text-center table-bordered table-striped" id="keranjangTable">
                <thead class="table-dark">
                    <tr>
                        ...
                        @if ($detailPeminjaman->filter(function($item) {
                            ...
                            <th class="fs-6 fw-semibold text-center align-middle" scope="col" width="">Nomor Pinjaman</th>
                ...
                <tbody>
                    ...
                    <tr>
                        ...
                        <td class="align-middle fs-6">{{ $itemPeminjaman->buku->judul ?? 'Data Buku tidak ditemukan' }}</td>
                        <td class="align-middle fs-6">{{ $itemPeminjaman->buku->penulis ?? 'Data Buku tidak ditemukan' }}</td>
                        <td class="align-middle text-center fs-6">{{ $itemPeminjaman->jumlah }}</td>
                        <td class="align-middle fs-6">Rak {{ $itemPeminjaman->buku->rak->rak ?? 'Data rak tidak ditemukan' }} - Baris {{ $itemPeminjaman->buku->rak->baris ?? '' }}</td>
                        @if (in_array($itemPeminjaman->status_peminjaman, ['menunggu', 'dipinjam', 'dibatalkan', 'terlambat']))
                            ...
                            <td class="align-middle text-center fs-6">{{ date('d-m-Y', strtotime($itemPeminjaman->tanggal_pinjam)) }}</td>
                            <td class="align-middle text-center fs-6">{{ date('d-m-Y', strtotime($itemPeminjaman->tanggal_kembali)) }}</td>                            <td class="align-middle fs-6">{{ $itemPeminjaman->denda ?? '-' }}</td>
                ...
                <div class="col-md-4">
                    <form action="{{ route('keranjang.ajukan') }}" method="POST">
                        ...
                        <div class="input-group-lg mb-3">
                            ...
                            <input type="date" class="form-control" id="tanggalpinjam" name="tanggal_pinjam" value="{{ date('Y-m-d') }}" required>
                        </div>
                        <button type="submit" class="btn btn-primary px-5 py-2 fs-5 fw-semibold w-100">Ajukan Peminjaman</button>
                    </form>
                </div>
            </div>
        @endif
